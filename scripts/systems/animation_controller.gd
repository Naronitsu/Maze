...
@onready
var animated_sprite: AnimatedSprite2D = get_node_or_null(animated_sprite_path) as AnimatedSprite2D
...
func _process(delta: float) -> void:
	...
	if animated_sprite != null and _anim_name != &"" and _mode == Mode.PROGRESS_STEPS:
		...
		if frame_count > 0:
			...
			if frame_count != desired and OS.is_debug_build():
				push_warning(
					(
						"[AnimationController] '%s' has %d frames; expected %d for stepped progress"
						% [_anim_name, frame_count, desired]
					)
				)
	...
	if animated_sprite != null and _anim_name != &"":
		...
		if (
			...
			and animated_sprite.sprite_frames.has_animation(_anim_name)
		):
			animated_sprite.frame = max(
				0, animated_sprite.sprite_frames.get_frame_count(_anim_name) - 1
			)
