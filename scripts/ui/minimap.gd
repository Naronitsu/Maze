...
func _ready() -> void:
	...
	print(
		(
			"[Minimap] References validated: layer=%s, fog=%s, player=%s"
			% [layer != null, fog != null, player != null]
		)
	)
...
func _initialize_minimap() -> void:
	...
	print(
		(
			"[Minimap] Initialization complete, cell_size=%s, marker visible=%s"
			% [_cell_size, player_marker.visible]
		)
	)
...
func _render_static_map() -> void:
	...
	if (
		fog != null
		and fog.explored_viewport != null
		and fog.maze_size_world.x > 0.0
		and fog.maze_size_world.y > 0.0
		and fog.tile_size.x > 0.0
		and fog.tile_size.y > 0.0
	):
	...
	if map_pixel_width <= 0 or map_pixel_height <= 0:
		push_warning(
			"[Minimap] Invalid pixel dimensions: %dx%d" % [map_pixel_width, map_pixel_height]
		)
		return
	...
	print(
		(
			"[Minimap] Static map rendered, texture size: %s, has texture: %s"
			% [img.get_size(), map_display.texture != null]
		)
	)
	...
	for y in range(_maze_bounds.size.y):
		for x in range(_maze_bounds.size.x):
			...
			if layer.has_method("is_door_cell"):
				...
				if (
					is_door
					and layer.has_method("is_door_open")
					and bool(layer.call("is_door_open", cell))
				):
					door_color = door_open_color
...
func _update_player_marker() -> void:
	...
	if (
		fog != null
		and fog.explored_viewport != null
		and fog.explored_viewport.size.x > 0
		and fog.explored_viewport.size.y > 0
	):
	...
	if _mode == MinimapMode.ZOOMED:
		...
		player_marker.position = (
			origin
			+ Vector2(relative_x * map_display.size.x, relative_y * map_display.size.y)
			- half_marker
		)
...
func _on_door_opened(cell: Vector2i) -> void:
	...
	if (
		local_x < 0
		or local_x >= _maze_bounds.size.x
		or local_y < 0
		or local_y >= _maze_bounds.size.y
	):
		return
