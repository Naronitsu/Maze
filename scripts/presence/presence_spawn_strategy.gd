...
# ============================================================
class FarSpawnStrategy:
	extends PresenceSpawnStrategy
	var min_distance: int = 18
	var max_attempts: int = 800

	func _init(
		p_controller: GameController,
		p_maze: DungeonMazeLayer,
		p_min_dist: int = 18,
		p_attempts: int = 800
	) -> void:
	...
	func attempt(presence_node: PresenceRW) -> bool:
		...
		for attempt in range(max_attempts):
			var rand_cell: Vector2i = Vector2i(
				rng.randi_range(0, size.x - 1), rng.randi_range(0, size.y - 1)
			)
