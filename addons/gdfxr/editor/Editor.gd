...
func _ready():
	...
	popup.add_separator()
	popup.add_icon_item(
		get_theme_icon("ActionCopy", "EditorIcons"), translator.t("Copy"), ExtraOption.COPY
	)
	popup.add_icon_item(
		get_theme_icon("ActionPaste", "EditorIcons"), translator.t("Paste"), ExtraOption.PASTE
	)
...
func _notification(what: int):
	...
	match what:
		NOTIFICATION_ENTER_TREE, NOTIFICATION_THEME_CHANGED:
			find_child("ScrollContainer").add_theme_stylebox_override(
			...
			if extra_button:
				var popup = extra_button.get_popup()
				popup.set_item_icon(
					popup.get_item_index(ExtraOption.COPY),
					get_theme_icon("ActionCopy", "EditorIcons")
				)
				popup.set_item_icon(
					popup.get_item_index(ExtraOption.PASTE),
...
func _popup_file_dialog(
	mode: int, callback: Callable, default_filename := DefaultFilename.EMPTY
) -> void:
...
func _on_New_pressed():
	if _modified:
		_popup_confirm(
			translator.t("There are unsaved changes.\nCreate a new one anyway?"), _on_New_confirmed
		)
	else:
		_on_New_confirmed()
...
func _on_Extra_about_to_show():
	...
	popup.set_item_disabled(
		popup.get_item_index(ExtraOption.PASTE_JSFXR), not DisplayServer.clipboard_has()
	)
